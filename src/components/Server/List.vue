<template>
  <v-data-iterator :items="$store.state.server.list" :search="search" :sort-by="sortBy" :sort-desc="sortDesc" hide-default-footer disable-pagination :loading="$store.state.server.loading">
    ...
    <template v-slot:default="props">
      <v-lazy transition="fade-transition">
        <v-row class="mb-4">
          <v-col v-for="item in props.items" :key="item.serverName" cols="12" sm="6" md="4" lg="3">
            <v-card v-ripple @click="open(item.serverId, item)" flat>
      ...
      <v-fab-transition>
        <v-btn fab color="blue" dark :loading="$store.state.server.loading" v-show="$store.state.serverinfo.window === 0" @click="dialog" bottom right fixed>
...
export default {
  ...
  methods: {
    open(window, server) {
      ...
      this.$store.dispatch('serverinfo/setServer', server)
    },
    dialog() {
